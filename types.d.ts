...
declare module "@abap2ui5/linter/render" {
  ...
  export function openRenderer(opts?: {
    ...
    theme?: string;
    css?: boolean;
    /** Playwright's option of the same name (default true): Ctrl+C closes the
...
declare module "@abap2ui5/linter/compat" {
  /**
   * data/compat.json — the ecosystem compatibility record (generated by
   * scripts/generate-compat.mjs, gated by npm test). Which abap2UI5 framework
   * releases this linter line understands, and the UI5 range its snapshot
   * covers; read by app-template's pin checks and the VS Code extension.
   */
  export interface Compat {
    /** What the file is, who reads it and how it is regenerated. */
    note: string;
    /** This package's version (package.json). */
    linter: string;
    framework: {
      /** The oldest abap2UI5 release whose released client API the rules assume. */
      minimum: string;
      /** The abap2UI5 release the hand-maintained mirrors in lib/ were last synced against. */
      mirrored: string;
    };
    ui5: {
      /** The default UI5 version the gate judges against (`--ui5`). */
      floor: string;
      /** The `ui5Version` of data/properties.json — the gate knows nothing newer. */
      snapshot: string;
    };
  }
  const compat: Compat;
