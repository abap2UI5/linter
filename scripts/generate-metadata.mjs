...
/** `Key: "Value"` pairs of an object literal body - the RUNTIME values of the
 *  enum, which is what `isValid( )` tests and therefore what a setter and a
 *  bound value must carry. NOT what an XML view writes where the two differ:
 *  see enumKeys below. */
...
/** The XML-view spelling of the values whose KEY differs from them, as
 *  `{ "<runtime value>": "<key>" }` - and nothing at all for the overwhelming
 *  majority of enums, where the two are the same string.
 *
 *  UI5 enums are `{ Key: "Value" }` maps and the two spellings are NOT
 *  interchangeable; which one is right depends on how the value reaches the
 *  control:
 *
 *    XMLTemplateProcessor   parseValue( ) first, KEY -> value, then isValid( )
 *                           -> an attribute must be written as the KEY
 *    setProperty / binding  validateProperty -> isValid( ) alone, no parse
 *                           -> a setter argument must be the VALUE
 *
 *  and `isValid( )` is keyed by VALUE (`DataType.createEnumType`: mValues[
 *  sValue] = sName). So for `CalendarIntervalType.OneMonth = "One Month"` an
 *  XML view that writes the value gets `undefined` back from parseValue( ),
 *  an error in the log and the property's DEFAULT - while the key, which the
 *  demo kit has shipped for years, is the spelling that works.
 *
 *  Additive, like enumSince: a snapshot without this section simply judges
 *  every enum as if key and value matched, which is what it did before. */
function enumKeysOf(body) {
  const out = {};
  for (const m of stripComments(body).matchAll(/["']?(\w+)["']?\s*:\s*["']([^"']*)["']/g)) {
    const [, key, value] = m;
    if (value && key !== value && out[value] === undefined) out[value] = key;
  }
  return out;
}
...
function parseLibraryEnums(file, lib) {
  ...
  for (const m of src.matchAll(/thisLib\.((?:\w+\.)*\w+)\s*=\s*\{/g)) {
    ...
    const k = enumKeysOf(body);
    if (Object.keys(k).length) keys[`${lib}.${m[1]}`] = k;
  }
...
function parseModuleEnums(file, base) {
  ...
  for (const m of src.matchAll(/DataType\.registerEnum\(\s*["']([\w.]+)["']\s*,\s*(\w+)\s*\)/g)) {
    ...
    const k = enumKeysOf(body);
    if (Object.keys(k).length) keys[fullName] = k;
  }
  return { values: out, sinces, keys };
}
...
if (process.argv.includes('--check')) {
  ...
  console.log(
    ...
    `${Object.keys(enums).length} enums (${Object.keys(enumKeys).length} with a key that differs from its value)`
  );
}
