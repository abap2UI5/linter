...
- Input is **generic-builder classes** — `z2ui5_cl_ui5_view_builder`
  ...
  `<class>=>factory`, plus every app class (`INTERFACES z2ui5_if_app`) even
  when its view is built in another class — such a class is judged by the
...
| --- | --- |
| `lib/properties.mjs` | `unknown-control`, `control-too-new`, `control-deprecated`, `sapui5-only-control` (an error with `--distribution openui5`, a hint with no `distribution` configured, silent with `sapui5`), `deprecated-library` (the five libraries SAP deprecated WHOLE, which is the one thing here that cannot come from the snapshot: a library nobody should write any more is not in what the harvest reads, so a `sap.ui.commons` view was invisible rather than merely unjudged — and it stands the rule above it down, because a tag that has to be rewritten does not also need advice on the distribution it is kept under), `unknown-property`, `member-too-new`, `member-deprecated`, `event-parameter-too-new`, `unknown-event-parameter`, `invalid-property-value`, `unknown-aggregation`, `aggregation-in-aggregation`, `too-many-children`, `invalid-aggregation-child`, `duplicate-aggregation`, `missing-required-aggregation`, `duplicate-id`, `undeclared-namespace`, `invalid-expression-binding`, `binding-for-event`, `event-for-property`, `unknown-binding-path`, `collection-bound-to-property`, `binding-type-mismatch`, `json-bind-on-scalar-property`, `uncurated-formatter` (list: `lib/formatters.mjs`), `binding-on-association`, `unknown-model`, `event-on-disabled-control`, `raw-javascript-to-frontend` (view half; the `follow_up_action` half emits in `abap-rules.mjs`), `missing-accessibility`, `aggregation-too-new` (the aggregation-TAG half of `member-too-new`, split off because UI5 resolves an unknown tag as a control class and the 404 kills the view), `toolbar-control-in-bar`, `relative-aggregation-without-context` (the aggregation counterpart of `relative-binding-without-context`), `json-literal-in-attribute`, `picker-value-without-format`, `relative-asset-url`, `date-type-without-source`, `enum-value-too-new`, `validating-setter-out-of-range` (the harvested `setterMin`: a property whose own setter refuses the ABAP initial 0), `unresolved-attribute-value` (an enum-typed attribute the reconstructor had to drop, so the one check that would have decided it went with the value), `insecure-asset-url` (the `http://` sibling of `relative-asset-url` in the same `checkValue` site — an error where the browser LOADS the URI, a per-finding `hint` where it navigates: `href` and the ObjectHeader's `titleHref`/`introHref`), plus the DOCUMENT-level pass `checkDocument( )` that runs once after the walk: `unused-namespace-declaration` (the mirror of `undeclared-namespace`, reading tag prefixes AND attribute prefixes — `core:require`, `app:type` — with a deleting `--fix` attached in `findings.mjs` only when the `a( )` call has its line to itself), `association-unknown-id` (the other half of `binding-on-association`: a plain id no control of the document carries, judged against the document's literal ids and only when every id in it is one), `column-cell-count-mismatch` (a `sap.m.Table` whose row template has a different number of cells than the table has columns — mapped by index, nothing logged), `bound-aggregation-without-template` (a bound aggregation with no child to clone — `bindAggregation` throws and the view never loads), `unknown-source-property` (the `$source>/name` twin of `unknown-event-parameter`, recorded by the reconstructor as `sourceParams`), `editable-control-without-binding` (an input whose value property is neither bound nor read back by an event — a hint, judged per family through `INPUT_FAMILIES`), `undefined-css-class` (a `class` token neither under a UI5 prefix in `lib/css-classes.mjs` nor defined by a `<style>` block the same document carries; a document whose stylesheet is unresolved is not judged), `external-link-without-target` (an absolute `href` with no `target` on the same element, prefixed pairs judged against their own `…Target`), `second-root` (a second element at the document's top level — the split chain after a standalone `factory( ).`, which the render gate rejected and the property gate never named) |
...
| `lib/abap-rules.mjs` | `non-released-api` (list: `lib/released-api.mjs`), `obsolete-binder`, `obsolete-model-update`, `obsolete-frontend-event`, `binding-to-local`, `binding-to-nonpublic`, `binding-to-static` (a `_bind( )` on a `CONSTANTS` or `CLASS-DATA`, in any section: the framework's `attri_search` filters on `is_class = abap_false AND is_constant = abap_false`, so the bind finds nothing and raises BINDING_ERROR — `binding-to-nonpublic` stands down for a static root, since PUBLIC would change nothing), `binding-to-reference`, `unconverted-abap-boolean`, `unescaped-text-in-attribute` (the text twin of the boolean rule: a value whose ORIGIN is data — a template that interpolates, a name whose assignments show no binding vocabulary — written through `v`, where a brace in it is a binding; the fix moves it onto the builder's `t`. Three things are NOT data: a `CONSTANTS`, an IMPORTING parameter every call site in the class passes a literal-only value — judged per method and parameter, since `text` is a parameter of more than one helper — and a value on a property whose type can show no text at all: `int`, `boolean`, a CSS size or colour, an id, a URI, an enum, resolved through the element literal nearest before the `a( )` call and the class's own `xmlns` declarations), `event-without-handler` (a handler is a `WHEN`, a `check_on_event( )`, or a comparison of `get_event( )` against a literal WHEREVER it stands — the one-branch `IF` abaplint's `short_case` forces, its `ELSEIF`/`OR` continuations, `EQ`, and the literal on the left), `event-arg-unresolved`, `event-arg-out-of-range`, `event-arg-js-callback`, `event-arg-single-row-table` (a `t_arg = VALUE #( ( x ) )` on a `client->_event( )` that carries ONE value, where `arg = x` is the same string_table - silent from two rows on, when both spellings are passed, and on a row containing a `#`, since `arg` is generic and `CONV #( )` has no type to derive there; fixable), `trailing-empty-event-arg`, `enum-field-unset-on-insert`, `unescaped-brace-in-style`, `collapsed-brace-in-style`, `escaped-brace-in-backtick`, `unused-public-attribute`, `popover-display-val`, `popover-anchor-unknown-id`, `hardcoded-binding-path`, `live-event-roundtrip`, `get-viewname-removed`, `ui5-internal-access`, `commercial-ui5-host` (scoped to a RUNTIME load — a demo-kit link and a `/test-resources/` asset are not one, and reporting them is what made consumers switch it off wholesale), `source-line-too-long`, `obsolete-bind-argument`, `abap-date-formatter-mismatch`, `escape-sequence-in-backtick`, `private-app-attribute`, `popup-without-close-wire` (a `Dialog` built in a method that wires no event at all — per METHOD, a hint), `event-name-case-mismatch` (a raise spelled differently from its one handler — `get_event( )` compares letter for letter, and `event-without-handler` is case-blind on purpose, so this is the rule that sees `save` against `SAVE`; fixable), `handler-without-event` (the inverse of `event-without-handler` — a handler nothing raises, judged only when every raise and every handler is a literal, the class raises at least one event itself and never hands its client to another object; a hint), `binding-to-expression` (a `_bind( )` on a literal, a method call or a constructor expression — no attribute, no path, a runtime exception; an error), `loop-work-area-bound` (a `_bind( )` on the work area of a `LOOP AT` — one path for every row, or a path the model never carries; fixable in the ASSIGNING shape into the framework's cell binding `tab = … tab_index = sy-tabix`), `message-box-removed-parameter` (the pure UI5 options abap2UI5#2748 took off `message_box_display( )` / `message_toast_display( )` — `MESSAGE_BOX_REMOVED` / `MESSAGE_TOAST_REMOVED`; the class does not compile), `smart-variant-without-init` (a SmartVariantManagement in the id map and no `cs_event-smart_variant_init` wire anywhere in the class), `absent-boolean-overrides-default` (the boolean twin of `enum-field-unset-on-insert`, and it needs a SECOND signal: the seed has to be inconsistent), plus the activation family that follows the `source-line-too-long` precedent — `value-header-default-reassigned`, `into-corresponding-inline-decl`, `class-constructor-visibility`, `redundant-conv-i`, `redundant-serializable` (`INTERFACES if_serializable_object.` beside `INTERFACES z2ui5_if_app.`, which already includes it — read out of the INTERFACES STATEMENTS rather than with the usual guard, so the chained `INTERFACES: if_serializable_object, z2ui5_if_app.` is seen too; a hint, fixable where the line is its own statement), and the 2026-09 additions from the same staging area: `empty-catch-block` (SLIN's empty-handler finding, a hint), `boolc-instead-of-xsdbool` (the downport dialect), `delete-index-in-loop` (the loop-cursor runtime trap, an error), and the four the 2026-09-12 round promoted out of samples-controls's pattern-lint into the same `checkAbapHygiene( )` so `--all-classes` reaches them: `default-key-table` (a bare `TYPE [STANDARD] TABLE OF` with no `WITH … KEY`), `abapdoc-html-tag` (a raw `<tag>` in a `"!` line, read from the RAW source because the scrubbed copy has the comment blanked; the ABAP Doc markup tags exempt), `event-arg-default-index` (`get_event_arg( 1 )` / `( v = 1 )`, with a deleting fix), `client-handle-capture` (a `_bind( )`/`_event( )` handle assigned to a variable — judged on STATEMENTS, since a `v = client->_bind( )` continuation line starts a line too) — plus `unbound-public-attribute` beside `unused-public-attribute` (the same PUBLIC surface, the other half: used in ABAP, bound by nothing; kept a separate rule because the remedy is PROTECTED, not deletion) — plus the abapGit round-trip family riding the `source-line-too-long` walk: `byte-order-mark`, `crlf-line-ending`, `trailing-whitespace`, `missing-final-newline` (all warnings — the file never stops diffing, but it imports; all four fixable) — plus `checkAbapRules( )` itself, the entry point every ABAP-side rule is called from and the order they run in |
...
| `lib/lifecycle-rules.mjs` | what the class does ACROSS a roundtrip: `view-never-displayed`, `missing-view-display-on-navigated`, `missing-on-navigated-branch`, `separate-lifecycle-ifs`, `manual-init-flag`, `duplicate-for-iterator`, `redundant-init-display` (the INVERSE of the two rules above it: `check_on_init( )` implies `check_on_navigated( )`, so an `OR` of the two and a fork whose arms are the same display call both decide nothing), `lifecycle-is-initial`, and the three FLOW rules over `branchTail( )` — `unconditional-popup-display` (a popup at the top level of `main( )`, rebuilt on every roundtrip), `display-after-nav-app-call` (a display behind the hand-over, usually a missing RETURN), `double-display-in-branch` (the same slot twice in one sequence, the first dead). Also split out of `abap-rules.mjs`, and called from exactly the points its code sat at: `report( )` collapses repeats and the FIRST finding of a shape wins, so the call ORDER is part of the output |
| `lib/guide-rules.mjs` | five sentences of the app guide (`docs/agents/building-apps.md` §5/§6) and of `z2ui5_if_client`'s ABAP Doc, read as rules through one entry point `checkGuideRules( )` called from `checkAbapRules( )`: `frontend-action-as-backend-event` (a `cs_event-` constant handed to `_event( )` in any spelling — a backend event named `POPUP_CLOSE` that closes nothing, and the one dead wire `event-without-handler` cannot judge because the name is not a literal; fixable, the rename to `follow_up_action( val = … )`), `popup-display-xml` (the mirror of `popover-display-val`; fixable), `queue-last-without-no-busy` (a `live*` wire — `isLiveEvent( )` in `abap-source.mjs`, the same prefix `live-event-roundtrip` applies — whose `s_ctrl` sets `check_queue_last` without `check_no_busy`, so the busy overlay flashes over the field being typed into; a hint, the fix writes the flag into the existing `VALUE #( )`), `nest-view-without-destroy` (`nest_view_display( )` / `nest2_view_display( )` with no `method_destroy`, every call adding one more fragment; fixable only for the three inserts the ABAP Doc names, `NEST_DESTROY_BY_INSERT`), `omit-initial-drops-false` (`_bind( val = t omit_initial = abap_true )` where the row declares an `abap_bool` the template binds to a property defaulting to `true` — read off the same `boolFields` map `absent-boolean-overrides-default` uses, which stands down under `omit_initial`; no fix). Every one probed silent on 0.7.0 and measured at 0 on abap2UI5's own app classes before shipping |
| `lib/abap-source.mjs` | no findings — the readers those three share (`literalElements`, `methodSpans`, `displayPathMethods`, `bindableAnywhere`, `ifBranchEnd`, `ifBlockEnd`, `branchTail` — the rest of a statement sequence on its own level, nested blocks blanked, which is what a "runs whenever this runs" question reads). Pure functions of a scrubbed source, so no module has to learn about another |
...
| `lib/reconstruct.mjs` | `excess-shut`, `duplicate-property`, `attribute-without-element`, `display-root-mismatch`, `chain-unbalanced-parens` (a chain with one `)` too many or one `(` never closed — a syntax error, and the one shape that makes `splitStatements` run the chain into the statement behind it, so the class used to reconstruct nothing and say nothing; read by a scan of its own, `noteUnbalancedParens`, that recovers at the stray paren), `open-levels` (note-only) — via `prep.structure`, consumed in `lib/index.mjs` |
...
| `lib/suggest.mjs` | no findings — `caseMatch( )`, the one did-you-mean the linter makes: the unique member of a closed set that is the written name up to letter case and `-`/`_`. Nine rules carry it as `written`/`suggestion` and a fix (`unknown-control`, `unknown-property`, `unknown-aggregation`, `invalid-property-value`, `unknown-event-parameter`, `unknown-icon`, `frontend-action-unknown-id`, `popover-anchor-unknown-id`, `unknown-directive-rule`); the view-side five get their span from `attachSuggestionFixes( )` in `findings.mjs`, the others compute it where they report. The CLI's `--explain` and the config loader's `unknown rule` error use it for a mistyped rule id against the same closed set. No edit distance, on purpose: `Buttom` → `Button` is a guess, `button` → `Button` is not |
...
| `lib/findings.mjs` | the **severity/wording/position layer** (`severityOf`, `SEVERITIES`, `RULES`, messages) plus the two things a repo can say back to it: `applyRules` (the config's `rules` block) and `applyDirectives` (`abap2ui5lint-disable-*` comments) — and the two findings about the directives themselves, emitted from `applyDirectives` because that is the only place that knows what a directive did: `unknown-directive-rule` (an id not in `RULES`, a warning, with the `caseMatch( )` did-you-mean and a fix that rewrites the id; `render-error` gets its own sentence, since a directive can never address a render failure) and `unused-directive` (a directive that suppressed nothing — per id for a list, as a whole for a bare one; ESLint's `--report-unused-disable-directives`, a hint, with a deleting fix on the line forms). Both go through `applyRules` and the directives like any other id, with one asymmetry: a BARE directive cannot waive its own finding, `" abap2ui5lint-disable unused-directive` may. Every consumer (CLI, VS Code extension, samples-controls `view-gates`, mcp-server) reads what a finding *means* from here; a new finding type needs its severity classified here or consumers fall back to a default |
...
builder's factory and, since 2026-09-25, a class that declares
`INTERFACES z2ui5_if_app` without one (the app whose view a views class or a
generator builds — before that it was dropped even when named on the command
line, and `node cli.mjs zcl_app.clas.abap` answered `no checkable app classes`
with exit 0) — anything else is dropped, and a dropped file leaves the run
...
`checkAbapHygiene( )`, the released-API check. An APP class that builds no view
gets those plus the ABAP-side rules that read the class and not the view
(`VIEWLESS_APP_RULE` in `lib/index.mjs`: the `binding-to-*`, `obsolete-*` and
`event-*` families, `private-app-attribute`, the lifecycle and flow rules), is
marked `appWithoutView` on its result, counted in the run summary, and named
under a one-file report — a clean verdict on it is a verdict on the class, not
on a view. A source-side rule that belongs
...
(`lib/css-classes.mjs` is a sixth, of a different kind: the CSS-class PREFIX
families UI5's own stylesheets define, harvested once from the OpenUI5
sources — the file says how — and not watched by `check-upstream`, because
its source is UI5 and a family moves only when SAP starts a library)
...
| Untrusted text rendered through `a( v = … )` (the ui5-check entry that was "blocked on origin") | `unescaped-text-in-attribute` — the origin is read out of the class: every statement that writes the name, so a binding assembled in a variable stays silent and a value nothing assigns is data |
...
Keep that distinction if the rule is ever widened. (It was widened on
2026-09-12, and as a SEPARATE rule for exactly that reason:
`unbound-public-attribute` reports the used-but-unbound half with the
remedy PROTECTED — the draft persists the whole object whatever the section,
only the MODEL filters on visibility — while `unused-public-attribute` keeps
saying "delete it". Two remedies, two rules.)
...
| Origin | Rule |
| --- | --- |
| the framework gained `_bind` parameters the reconstructor did not know — an `omit_initial_paths`/`json` bind DROPPED its attribute from the reconstructed view, blinding every gate to it | `bindingOf` now parses named args; shape-neutral params (`omit_initial`, `omit_initial_paths`, `json`, `view`) reconstruct as the plain binding, shape-CHANGING ones (`custom_mapper`, `switch_default_model`) still stay unresolved on purpose. **`tab`/`tab_index` left that list in 2026-08-29**: the cell binding's path IS computable (`val = t[ n ]-field` → `/T/(n-1)/FIELD`), and leaving it unresolved was the same blindness one row up — see the CHANGELOG entry for the consistency conditions |
...
The 2026-09-12 round took the five checks samples-controls's `pattern-lint.mjs`
still carried that are NOT corpus policy — each rests on a fact about ABAP,
ABAP Doc, the framework or this linter's own reconstructor — plus four view
rules nothing had asked for yet, and closed the one duplicate the corpus gate
kept only to know a dispatcher shape:

| Origin | Rule |
| --- | --- |
| pattern-lint `unbound-public-attribute` (38 attributes in 23 ports moved on 2026-09-11) | `unbound-public-attribute` — a warning; the doc says why it is not a widening of `unused-public-attribute` |
| pattern-lint `default-key-table` (slipped abaplint's `default_key`, which sees only the EXPLICIT form; app 034) | `default-key-table`, in `checkAbapHygiene( )` |
| pattern-lint `abapdoc-html-tag` | `abapdoc-html-tag` — over the RAW source, with the ABAP Doc markup tags exempt (pattern-lint's regex excluded them by accident: a tag with an attribute has a space) |
| pattern-lint `event-arg-default-index` | `event-arg-default-index`, a hint with a deleting fix — `v TYPE i DEFAULT 1` is the signature |
| pattern-lint `client-handle-capture` (a human decision, apps 005/053/007) | `client-handle-capture` — it turned out to be a LINTER fact, not taste: a captured handle is a runtime value to the reconstructor, so the attribute written from it is dropped from every gate's view of the class with no finding at all (measured: `DATA(lv_b) = client->_bind( mv_text )` then `v = lv_b` reconstructs `<Input/>` without its value). A warning for that |
| pattern-lint `dead-event-wire` — a duplicate of `event-without-handler` kept only because it knew the IF-shaped dispatcher | `event-without-handler` already read `IF client->get_event( ) = …`, `ELSEIF` and `OR` (the comparison is found wherever it stands); what it did NOT read were the operator spelled `EQ` and the literal on the LEFT, both taught now with a fixture per spelling, so the duplicate can be retired downstream |
| the mirror nobody had written of `undeclared-namespace` | `unused-namespace-declaration` — tag AND attribute prefixes, the default `xmlns` never; `--fix` only where the `a( )` call has its line to itself |
| ported samples whose CSS file was not ported with them | `undefined-css-class` + `lib/css-classes.mjs` (the UI5 prefix families, harvested from the OpenUI5 sources — 5,148 distinct class tokens, of which sapM/sapUi/sapF/sapUxAP/sapTnt cover 5,069) |
| an abap2UI5 app is ONE page | `external-link-without-target` — a same-window navigation replaces the runtime, every view and the session |
| every system but a laptop serves https | `insecure-asset-url` — the `http://` sibling of `relative-asset-url`, error where the browser BLOCKS the load (mixed content), a per-finding `hint` for a hyperlink |

Measured on the samples-controls corpus (637 classes) before shipping: 92
classes `unused-namespace-declaration`, 15 `undefined-css-class` (app 345 is
the "carries a stylesheet that defines OTHER tokens" case: its `<style>`
defines `.exampleDiv` while eleven controls wear `gridWrapper`, a class the
original used as a jQuery hook), 3 `external-link-without-target`, 12
`insecure-asset-url` (11 `http://sap.com` hyperlinks as hints, one wikimedia
image as the error), 0 `default-key-table`, 0 `abapdoc-html-tag`, 0
`event-arg-default-index`. Two of the ports came back NON-zero where
pattern-lint said zero, and both times the port was right and the corpus gate
wrong: `unbound-public-attribute` found three (two of them excused by
pattern-lint because its model-path regex `/name/` had matched the attribute
name inside a COMMENT — this linter reads the scrubbed source — and one in
`src/03`, which that gate never judged), and `client-handle-capture` found
one in the generated overview app, `press = client->_event( … )` into a
declared local later written as `v = press` — pattern-lint matched only the
inline `DATA( )` form, and the assignment form drops the attribute exactly
the same way. `event-without-handler` reports five hints on the corpus, all
five pre-existing (the rule's code path for them did not change, and the
before/after runs are byte-identical) and all five deliberate roundtrip-only
wires — app 600 says so in a comment beside its `DRAG_START`.

The 2026-09-19 round drew the line the earlier rounds had only implied:
**a rule about ABAP as a language belongs in abaplint, not here.** The
pure-ABAP candidates of that round (a `REF #( )` into a generic target, a
`TYPE HANDLE` operand that is a call, `)->*` on 7.50, the generic-operand
traps of older releases, `INTO CORRESPONDING … @DATA( )` at v750,
`GET REFERENCE` under Cloud, the empty CATCH, the implicit default key, an
HTML tag in ABAP Doc, the file's last byte) went into abap2UI5's
`backlog/ABAPLINT.md` as paste-ready items, each measured on abaplint
2.120.52 first — and that measurement closed four the other way:
`class-constructor-visibility`, `value-header-default-reassigned` and
`boolc-instead-of-xsdbool` are `check_syntax` / `prefer_xsdbool` findings on
that version, and a test class without `LOCAL FRIENDS` is one too. They stay
here for consumers on older pins; do not add to that family. What shipped
here instead is what only an abap2UI5 reader can decide:
`handler-without-event`, `binding-to-expression`, `association-unknown-id`,
`column-cell-count-mismatch` (a fifth, `obsolete-event-constant`, was
withdrawn before release: abap2UI5 removed every constant under the
`"obsolete` label, and the label, three days later), six fixes on rules
that had none (`missing-on-navigated-branch`, `separate-lifecycle-ifs`, the
two section MOVES of `binding-to-nonpublic` / `private-app-attribute`,
`hardcoded-binding-path`, `external-link-without-target`) and the
did-you-mean on the closed sets that lacked it. Measured on abap2UI5's own
20 app classes and on app-template: 0 new findings, the existing 11
unchanged.
