...
1. **Property gate** — everything the view writes is resolved against a UI5
   ...
   **abap2UI5-specific rules** — the defects that stay *silent* at runtime,
   ...
   | `obsolete-frontend-event` | `client->_event_frontend( )` — superseded by `client->follow_up_action( )`, the same call under its current name. Under the old name the wire's `t_arg` is judged by none of the frontend-action rules |
...
| `obsolete-frontend-event` | `client->_event_frontend( … )` → `client->follow_up_action( … )`, arguments untouched |
