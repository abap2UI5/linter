...
ELSEIF client->check_on_navigated( ).
  " …
ENDIF.`,
    fixNote: 'Adjacent blocks are folded: an `ENDIF.` followed by nothing but the next lifecycle `IF` becomes one `ELSEIF`. A block that carries an `ELSE`, or statements between the two, is reported without a fix.',
  },
...
" … t_arg = VALUE #( ( \`messageView\` ) ( \`navigateBack\` ) )`,
  ...
  'relative-aggregation-without-context': {
    ...
    remedy: `)->a( n = \`items\` v = client->_bind( val = t_legend path = abap_true ) )`,
  },
