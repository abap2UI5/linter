...
view->a( n = \`title\` v = \`Hi\` )   " no element open yet`,
  ...
  'boolc-instead-of-xsdbool': {
    ...
    example: 'DATA(ok) = boolc( x > 1 ).     " write: xsdbool( x > 1 )',
  },
  'delete-index-in-loop': {
    category: 'structure',
    summary: '`DELETE itab INDEX sy-tabix` inside a `LOOP AT` over the same table',
    detail: 'Two failure modes, and only the loud one is reliably visible. Deleting the current row shifts every row behind it while the loop\'s own cursor walks on, so the row *after* each deletion is silently skipped — a filter that keeps rows it was told to drop, with nothing anywhere saying so. And where an inner construct (an inner LOOP that ended, a DO, a READ TABLE) has reset `sy-tabix`, the DELETE hits another table\'s index — or index 0, which is the `TABLE_INVALID_INDEX` short dump a live app answered HTTP 500 with. A sweep found the shape eight times across the ecosystem. ANY enclosing loop over the table counts, not only the innermost: the inner-loop case is the one that deleted by another table\'s index. Build the result instead — collect what is kept and assign it back — or `DELETE itab WHERE …`. The `DELETE itab INDEX sy-tabix` directly after a `READ TABLE`, outside a loop over that table, is correct and common, and is not reported.',
    example: 'LOOP AT t_rows INTO DATA(row).\n  IF row-obsolete = abap_true.\n    DELETE t_rows INDEX sy-tabix.   " skips the next row each time\n  ENDIF.\nENDLOOP.',
  },
  'byte-order-mark': {
    category: 'structure',
