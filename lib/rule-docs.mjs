...
export const RULE_DOCS = {
  ...
  'chain-call-per-line': {
    category: 'abap2ui5',
    summary: 'several builder calls on one line of a multi-line chain',
    detail: 'One call per line is what makes the indentation able to show the tree at all — a line holding three controls hides three levels of it. Reported only for a chain that is already written across several lines, so the compact one-liner form stays available; and closing calls do not count, because `)->shut( )->shut( ).` as a chain\'s last line is an established ending, not the readability problem this rule is about. A hint, like its neighbour.',
    example: ')->leaf( `Text` )->a( n = `text` v = `{TITLE}` )   " two calls, one line',
  },
  'obsolete-binder': {
    category: 'abap2ui5',
