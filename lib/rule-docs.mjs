...
" to downgrade the gate itself instead: rules: { 'render-error': 'warning' }`,
  ...
  'event-parameter-too-new': {
    ...
    example: `client->_event( val   = \`SEARCH\`
                t_arg = VALUE #( ( \`\${$parameters>/searchButtonPressed}\` ) ) )   " @since 1.114`,
    remedy: `client->_event( val   = \`SEARCH\`
...
DATA(html) = view->xml_escape( data ).`,
  ...
  'unconverted-abap-boolean': {
    ...
    remedy: `view->tag( \`Button\` )->a( n = \`visible\` b = flag )   " the builder renders true/false`,
...
" … WHEN \`PICK\`. client->get_event_arg( 2 )`,
    remedy: `client->_event( val = \`PICK\` t_arg = VALUE #( ( \`\${$source>/id}\` ) ) )
" … WHEN \`PICK\`. client->get_event_arg( 1 )`,
  },
...
ENDIF.`,
  ...
  'unresolved-attribute-value': {
    ...
    remedy: `a( n = \`state\` v = client->_bind( state ) )   " …and set STATE to a ValueState in ABAP`,
  },
...
ENDMETHOD.`,
  ...
  DATA(popup) = z2ui5_cl_ui5_view_builder=>factory( ).
  popup->ele( \`Dialog\` )->a( n = \`title\` v = \`Hi\` )->tag( \`Text\` )->a( n = \`text\` v = \`hello\`
      )->ele( \`buttons\` )->tag( \`Button\` )->a( n = \`text\` v = \`Close\` )->a( n = \`press\` v = client->_event( \`CLOSE\` ) ).
  result = popup->stringify( ).
ENDMETHOD.`,
  },
...
IF mv_ready = abap_false.`,
  ...
  'validating-setter-out-of-range': {
    ...
    example: `t_appointments = VALUE #( ( title = \`Review\` start_at = s ) ).   " recurrence_pattern unseeded -> 0`,
    remedy: `t_appointments = VALUE #( ( title = \`Review\` start_at = s recurrence_pattern = 1 ) ).`,
  },
  'absent-boolean-overrides-default': {
    ...
    remedy: `VALUE #( ( title = \`a\` showClose = abap_true )
         ( title = \`b\` showClose = abap_true ) )`,
  },
...
" …
  ...
  'manual-init-flag': {
    ...
    example: `IF check_initialized = abap_false.
...
" … t_arg = VALUE #( ( \`messageView\` ) ( \`navigateBack\` ) )`,
  ...
  'invalid-keyboard-shortcut': {
    ...
    remedy: `t_arg = VALUE #( ( \`Ctrl+Shift+S\` ) ( \`SAVE\` ) )`,
  },
  ...
  'enum-field-unset-on-insert': {
    ...
    remedy: `INSERT VALUE #( title = \`New\` type = \`None\` start_at = s ) INTO TABLE t.`,
  },
  ...
  'filter-groups-not-arrays': {
    ...
    example: `t_arg = VALUE #( ( \`d\` ) ( \`items\` ) ( \`filter\` )
                 ( \`[{"path":"NAME","operator":"Contains","value1":"x"}]\` ) )   " cleared`,
    remedy: `t_arg = VALUE #( ( \`d\` ) ( \`items\` ) ( \`filter\` )
...
" … )->a( n = \`percentValue\` v = client->_bind( percent ) )`,
  ...
  'picker-value-without-format': {
    ...
    example: `a( n = \`value\` v = client->_bind( start_at ) )   " no valueFormat: the picker cannot parse it`,
    remedy: `a( n = \`valueFormat\` v = \`yyyy-MM-dd'T'HH:mm:ss\`
  ...
  'date-type-without-source': {
    ...
    example: `a( n = \`text\` v = |\\{ path: '{ client->_bind( val = date path = abap_true ) }', type: 'DateType' \\}| )`,
    remedy: `a( n = \`text\` v = |\\{ path: '{ client->_bind( val = date path = abap_true ) }', type: 'DateType', formatOptions: \\{ source: \\{ pattern: 'yyyy-MM-dd' \\} \\} \\}| )`,
  },
...
ENDMETHOD.`,
  ...
  'settable-property-via-action': {
    ...
    remedy: `a( n = \`showSideContent\` v = client->_bind( show_side ) )`,
  },
