...
export const RULE_DOCS = {
  ...
  'aggregation-too-new': {
    ...
    example: 'view->ele( `Dialog` )->_generic( name = `footer` )  " @since 1.110 on a 1.71 target',
  },
  ...
  'unknown-icon': {
    ...
    example: 'view->leaf( `Button` )->a( n = `icon` v = `sap-icon://textFormatting` )',
  },
  'icon-too-new': {
    category: 'version',
    ...
    example: 'view->leaf( `Button` )->a( n = `icon` v = `sap-icon://information` )  " @since 1.80 on a 1.71 target',
  },
  'icon-removed': {
    category: 'version',
    summary: 'an icon that left the font again',
    detail: 'The font is nearly, but not quite, additive. `binary` arrived in 1.104 and is spelled `non-binary` from 1.120 on — the same codepoint, a renamed glyph — so a view written against a release in between renders no icon on every release after it. Since a target version is a **floor**, the app also runs on the releases where the name is gone, which is why this is reported independently of the target.',
  },
  'toolbar-control-in-bar': {
    ...
    example: 'view->_generic( name = `headerContent` )->leaf( `ToolbarSeparator` )',
  },
  'control-deprecated': {
    category: 'version',
