...
export const RULE_DOCS = {
  ...
  'obsolete-frontend-event': {
    category: 'abap2ui5',
    summary: '`client->_event_frontend( )` — superseded by `client->follow_up_action( )`',
    detail: 'The same call under its current name: `val`, `view` and `t_arg` are unchanged, and `follow_up_action( )` returns the handler expression too, so the rename holds in a view attribute as well as in a statement. Renaming also puts the call back within reach of the frontend-action rules (`unknown-frontend-action`, `invalid-frontend-action`, `frontend-action-unknown-id`, …), which key on the current names — under the old one its `t_arg` is judged by nothing.',
    example: 'client->_event_frontend( val = client->cs_event-set_title t_arg = VALUE #( ( `Hi` ) ) ).\nclient->follow_up_action( val = client->cs_event-set_title t_arg = VALUE #( ( `Hi` ) ) ).',
