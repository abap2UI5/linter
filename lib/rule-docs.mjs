...
view->tag( \`Text\` )->a( n = \`text\` v = client->_bind( title ) )`,
  ...
  'obsolete-event-constant': {
    category: 'frontend-wires',
    summary: 'a `cs_event` constant the interface keeps for compatibility only',
    detail: 'The run under the `"obsolete` label of `z2ui5_if_client=>cs_event` — `nav_container_to` and its four slot-prefixed siblings, `image_editor_popup_close`, `z2ui5`. Their values still travel (the server remaps the `*_nav_container_to` family into a `CONTROL_BY_ID` `to` wire), so a wire naming one WORKS today; what it lacks is a future: the constants are gone from the documentation site, and the deprecations page names each with its successor. A warning, no fix — the successor wire carries different arguments. Mirrored in `lib/frontend-actions.mjs` and gated by `check-upstream` against the label in the interface, so a constant that moves under it upstream reaches this rule.',
    example: `client->follow_up_action( val = client->cs_event-nav_container_to t_arg = VALUE #( ( \`nav\` ) ( \`page2\` ) ) ).`,
    remedy: `client->follow_up_action( val = client->cs_event-control_by_id t_arg = VALUE #( ( \`nav\` ) ( \`to\` ) ( \`page2\` ) ) ).`,
  },
