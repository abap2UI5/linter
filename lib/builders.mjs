...
/*
 * Whether a source calls a view builder's factory - the one test that decides
 * whether a class is CHECKED at all (collectFiles) and whether it is read as a
 * builder class (prepareAbap's usesBuilder).
 *
 * Case-insensitive, like ABAP itself outside its literals: a class written in
 * the pretty printer's "uppercase" setting spells it
 * `Z2UI5_CL_UI5_VIEW_BUILDER=>FACTORY( )`, and a byte comparison against the
 * lowercase name made such a class invisible - not judged wrongly, not
 * collected at all, which is the false green AGENTS.md warns about. The frozen
 * builder was always found this way; the current one was not.
 */
const FACTORY_RE = new RegExp(`\\b(?:${BUILDERS.map((b) => b.class).join('|')})\\s*=>\\s*factory\\b`, 'i');
export const usesBuilderFactory = (content) => FACTORY_RE.test(String(content));
...
export function dialectOf(content) {
  const used = BUILDERS.filter((b) => new RegExp(`\\b${b.class}\\s*=>\\s*factory\\b`, 'i').test(content));
  ...
  const dialect = Object.freeze({
    ...
    // the spelling is free: `Z2UI5_CL_UI5_VIEW_BUILDER => FACTORY` is the
    // same call, so every regex built from this carries the i flag
    factory: `(?:${classes})\\s*=>\\s*factory`,
    ...
    att: alt('att'),
    shut: alt('shut'),
    handleType: `TYPE\\s+REF\\s+TO\\s+(?:${classes})`,
