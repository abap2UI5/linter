...
export function checkLifecycleDispatch({ src, source, report }) {
   ...
   * each is a regex over that text and nothing more. */
  {
    ...
    for (const mm of src.matchAll(/\bMETHOD\s+z2ui5_if_app~main\s*\.([\s\S]*?)\bENDMETHOD\b/gi)) {
      const body = mm[1];
    ...
    for (const m of src.matchAll(/\b(?:me->)?\w*client\s*->\s*nav_app_call\s*\(/gi)) {
    ...
    for (const m of src.matchAll(DISPLAY)) {
      const from = afterStatement(m.index);
      if (from === -1) continue;
      const tail = branchTail(src, from);
