...
export function lifecycleIsInitial({ src, source, report, boolVars }) {
  ...
  const tailFix = (m, text) => {
    const tail = /\s+IS\s+(?:NOT\s+)?INITIAL$/i.exec(m[0]);
    if (!tail) return {};
  ...
  for (const m of src.matchAll(/(?:me->)?\w*client\s*->(check_on_\w+)\s*\(\s*\)\s+IS\s+(NOT\s+)?INITIAL\b/gi)) {
  ...
  for (const m of src.matchAll(/(?<![\w>-])(?:me->)?(\w+)\s+IS\s+(NOT\s+)?INITIAL\b/gi)) {
