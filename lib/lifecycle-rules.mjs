...
function navigatedBranchFix({ body, base, init, source, redisplays }) {
  ...
  const stmts = [];
  let depth = 0;
  let paren = 1;   // inside the `check_on_init(` the match ends on
  let start = 0;
  let first = true;
  ...
  const s = displays[0];
  const stmtText = source.slice(base + s.start, base + s.end).trim();
  ...
  const indent = stmtIndent.trim() === '' ? stmtIndent : `${ifIndent}  `;
