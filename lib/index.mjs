...
export async function screenshotFiles(files, opts = {}) {
  const { theme = 'sap_horizon', width = 1280, height = 900, fullPage = true } = opts;
  ...
  for (const file of files) {
    ...
    const isXml = /\.(view|fragment)\.xml$/.test(file) || /^\s*</.test(src);
    if (isXml) {
      jobs.push({ file, index: 0, xml: src, model: {}, kind: undefined });
      continue;
    }
    ...
    prep.docs.forEach((xml, index) => {
      jobs.push({ file, index, xml, model: prep.model, kind: prep.docKinds?.[index] });
    });
  }
  const shootable = jobs.filter((j) => j.xml);
  const out = jobs.map((j) => ({ file: j.file, index: j.index, kind: j.kind, png: undefined, errors: j.errors ?? [] }));
  if (!shootable.length) return out;
  progress({ phase: 'screenshot', done: 0, total: shootable.length });
  const renderer = await openRenderer({ theme, css: true });
  try {
    let done = 0;
    for (const job of shootable) {
      const shot = await renderer.screenshot({
        xml: job.xml, model: job.model, ...(job.kind ? { kind: job.kind } : {}), width, height, fullPage,
      });
      const entry = out.find((o) => o.file === job.file && o.index === job.index);
      entry.png = shot.png;
      entry.errors = shot.errors;
      progress({ phase: 'screenshot', done: ++done, total: shootable.length, file: job.file });
    }
  } finally {
    await renderer.close();
  }
