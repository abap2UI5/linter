...
export function checkNodes(root, { data, minUi5 = '1.71', allow = [], distribution = 'sapui5', model = null, shape = null, rootFields = null, models = null, jsonPaths = null } = {}) {
    ...
    if (node.name !== null && !isAggregationTag(node.name)) {
      ...
      if (full && knownControl(data, full)) {
        ...
        for (const [attr, value, at] of node.attrs) {
          ...
          if (models && isBinding(value)) {
            for (const mm of String(value).matchAll(/\{\s*([A-Za-z_][\w.]*)\s*>/g)) {
              if (!models.has(mm[1])) {
                report({ type: 'unknown-model', control: full, member: attr, value: mm[1], allowed: [...models].sort(), offset });
              }
            }
          }
