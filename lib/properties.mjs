 ...
 * does not claim to know its fields". `hasPath` already answers true for it
    ...
    if (node.name !== null && !isAggregationTag(node.name)) {
          ...
          if (bindings && !node.guessedAttrs?.has(attr)) {
            ...
            if (ctx && member.set !== 'aggregations') {
              for (const rp of relativeBindings(value)) {
                if (hasPath(ctx.row, rp)) continue;
                report({
      ...
      if (since && !withinFloor(since, floor)) {
        ...
        if (chainComplete(data, parent)) {
          ...
          const suggestion = caseMatch(node.name, [...memberNames(data, parent, ['aggregations', 'associations']), ...libControls]);
          report({
