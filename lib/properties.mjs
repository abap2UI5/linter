    ...
    if (node.name !== null && !isAggregationTag(node.name)) {
        ...
        for (const [attr, value, at] of node.attrs) {
          ...
          if (!member) {
            if (chainComplete(data, full)) {
              report({ type: 'unknown-property', control: full, member: attr, snapshot: data.__ui5Version, offset });
            }
            continue;
          }
      ...
      if (since && !withinFloor(since, floor)) {
        ...
        if (chainComplete(data, parent)) {
          report({ type: 'unknown-aggregation', control: parent, member: node.name, snapshot: data.__ui5Version, offset: node.offset });
        }
      } else {
