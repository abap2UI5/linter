 ...
 *  release would have to be added here by hand. (FRONTEND_EVENT_ALIASES and
 *  SERVER_EVENTS are not in the frontend source either, but they ARE a closed
 *  set upstream — `z2ui5_if_client=>cs_event` — and check-upstream reads
 *  that one.) */
...
export const FRONTEND_EVENTS = [
  ...
  'SET_ODATA_MODEL', 'STORE_DATA', 'DOWNLOAD_B64_FILE',
  'CROSS_APP_NAV_TO_PREV_APP', 'CROSS_APP_NAV_TO_EXT', 'LOCATION_RELOAD',
  'HASH_BACK', 'SYSTEM_LOGOUT', 'OPEN_NEW_TAB',
  ...
  'Z2UI5', 'PLAY_AUDIO', 'SMART_VARIANT_INIT',
 ...
 *  not gated against the dispatch table — but every one of them is a released
 *  cs_event constant an app is meant to use, so the linter must accept them,
 *  and check-upstream gates them the other way round: every value of
 *  `z2ui5_if_client=>cs_event` has to be in FRONTEND_EVENTS, here or in
 *  SERVER_EVENTS, and every name here has to still be a cs_event value.
 *
...
/** The cs_event constant NAMES upstream keeps for compatibility only — the
 *  run under the `"obsolete` label in `z2ui5_if_client=>cs_event`, which the
 *  interface's own documentation site drops and whose deprecations page names
 *  each with its successor. Their VALUES still travel (the server remaps the
 *  five `*_NAV_CONTAINER_TO` names into a CONTROL_BY_ID `to` wire, see
 *  FRONTEND_EVENT_ALIASES), so a wire naming one WORKS today; what it lacks
 *  is a future, and that is what `obsolete-event-constant` says. Constant
 *  names, lower-case, because that is what a class writes
 *  (`client->cs_event-nav_container_to`); the wire value is the alias list's
 *  business. Gated by check-upstream against the label in the interface. */
export const OBSOLETE_EVENT_CONSTANTS = [
 ...
 *  a CASE over these five and RETURNS, so they queue no action of their own
 ...
 *  from all of them together. HASH_REPLACE (`cs_event-hash_replace`, UI5's
 *  replaceHash: no new history entry) and HASH_ATTACH_CHANGED
 *  (`cs_event-hash_attach_changed`, attachHashChanged: a backend event for
 *  foreign hash changes) are the hash_* family's two newer members and were
 *  missing here, so the wire the documentation shows was an unknown action.
 *
 ...
 *
 *  Not in the frontend source, so — like FRONTEND_EVENT_ALIASES — gated by
 *  check-upstream against `z2ui5_if_client=>cs_event` rather than against the
 *  dispatch table. */
export const SERVER_EVENTS = [
  'SET_NAV_ROUTING', 'SET_PUSH_STATE', 'SET_APP_STATE_ACTIVE',
  'HASH_REPLACE', 'HASH_ATTACH_CHANGED',
];
 ...
 *  line, the two variant-init actions log only after five seconds of retries,
 *  BINDING_CALL logs "no binding on control". Same trust condition as
 *  CONTROL_BY_ID: judged only when every id the class declares is a literal. */
