 ...
 *
 *     view->ele( n = `View` ns = `mvc`         <- the anchor line
 *         )->a( n = `xmlns` v = `sap.m`        <- inside View
 *         )->ele( `Page`                       <- a child of View
 *             )->a( n = `title` v = `My App`   <- inside Page
 *             )->tag( `Input`                  <- a child of Page
 *                 )->a( n = `value` v = …      <- inside the Input
 *             )->tag( `Button`                 <- back out: a SIBLING of Input
 *         )->end( ).
 *
 ...
 * parent's column (`)->ele( `Shell` )` then `)->ele( `Page` )`, the standard
 ...
 * `)->tag( `Text` )->a( n = `text` v = `{TITLE}` )` is one control with its
 ...
 *
 * Closing calls do not count either: `)->end( )->end( ).` as a chain's last
...
function classify(text) {
  ...
  while (i < text.length) {
    ...
    if (c === '`' || c === "'") {
      mask[i++] = LIT;
      while (i < text.length) {
        if (text[i] === c) { mask[i++] = LIT; if (text[i] === c) { mask[i++] = LIT; continue; } break; }
        if (text[i] === '\n') break;
        mask[i++] = LIT;
      }
      continue;
...
export function checkChainHouseLayout(source, d) {
  ...
  const findings = [];
  const kindRe = new RegExp(`^\\)->\\s*${d.verbs}\\s*\\(`, 'i');
  const anyVerbRe = new RegExp(`->\\s*${d.verbs}\\s*\\(`, 'i');
  ...
  for (const stmt of splitStatements(scrubbed)) {
    ...
    const headRole = (() => {
      const ms = [...src.slice(lo, pts[0]).matchAll(new RegExp(`(?:->|=>)\\s*${d.verbs}\\s*\\(`, 'gi'))];
