...
export function bindingOf(expr) {
  ...
  if (/^\s*(?:me->)?\w+(?:-\w+)*\s*$/.test(body)) {
    ...
    if (cellArgs) {
      ...
      if (!cell || args.tab === undefined || args.tab_index === undefined) return null;
      if (!/^\d+$/.test(args.tab_index)) return null;
      const row = Number(args.tab_index);
      if (row < 1 || row !== Number(cell[2])) return null;
      if (args.tab.replace(/^me->/, '') !== cell[1]) return null;
...
function parseData(content) {
  ...
  for (const m of content.matchAll(/^\s*DATA\s+(\w+)\s+TYPE\s+(?:(STANDARD TABLE OF\s+((?:\w+(?:=>|~))?\w+))|REF TO\s+[\w=>~]+|((?:\w+(?:=>|~))?\w+))\s*(?:LENGTH\s+\d+)?\s*(?:DECIMALS\s+\d+)?\s*(?:WITH EMPTY KEY\s*)?(?:VALUE\s+(`(?:[^`]|``)*`|abap_true|abap_false|-?\d+(?:\.\d+)?))?\s*\.\s*$/gm)) {
