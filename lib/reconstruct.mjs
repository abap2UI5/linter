 ...
 * field naming it. The name it is filed under is qualified with its parent, so
 * two parents can each have an `s_details` without one shadowing the other -
 * nothing outside reads these names, they only have to resolve, and they are
 * spelled with characters a type name may carry so the field matcher accepts
 * the field that names them. Innermost first, so a block nested two deep is
 * lifted before the block containing it. */
function liftNested(parent, body, structs) {
  const INNERMOST = /BEGIN OF (\w+)(?:\s+READ-ONLY)?\s*,((?:(?!BEGIN OF )[\s\S])*?)END OF \1\s*,/;
  let out = body;
  for (let m = INNERMOST.exec(out); m; m = INNERMOST.exec(out)) {
    const key = `${parent}__${m[1]}`;
...
function parseTypes(content) {
  ...
  for (const m of content.matchAll(/BEGIN OF (\w+)(?:\s+READ-ONLY)?\s*,([\s\S]*?)END OF \1/g)) {
    ...
    const body = liftNested(m[1], m[2], structs);
...
export function prepareAbap(abapSource) {
  ...
  return {
    nodes, docs, docKinds, model, modelShape, notes, helperTokens, rootFields, jsonPaths,
