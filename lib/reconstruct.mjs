...
export function makeResolver(content, boundVars, notes, bindMeta = null) {
  ...
  for (const m of content.matchAll(new RegExp(
    String.raw`(?:^|\s)(?:DATA\()?(\w+)\)?\s*=\s*(${LIT}(?:\s*&&\s*${LIT})*)\s*\.(?=\s|$)`, 'g'))) {
...
function applyToken(role, body, stack, resolveExpr, notes, structure, offset, d) {
  ...
  const nsm = body.match(/(?:^|\s)ns\s*=\s*([`'])([^`']*)\1/);
...
function parseData(content) {
  ...
  for (const m of content.matchAll(new RegExp(String.raw`^\s*DATA\s+(\w+)\s+TYPE\s+(?:((?:STANDARD|SORTED|HASHED)\s+TABLE OF\s+((?:\w+(?:=>|~))?\w+))|REF TO\s+[\w=>~]+|((?:\w+(?:=>|~))?\w+))\s*(?:LENGTH\s+\d+)?\s*(?:DECIMALS\s+\d+)?\s*(?:WITH\s+[^.]*?)?\s*(?:VALUE\s+(${LITERAL}|abap_true|abap_false|-?\d+(?:\.\d+)?))?\s*\.\s*$`, 'gm'))) {
...
function parseRows(region, rowType, types, shape = false) {
  ...
  return rows.map((rowSrc) => {
    ...
    const pairRe = new RegExp(String.raw`(\w+)\s*=\s*(${LITERAL}|VALUE #\s*\(|abap_true|abap_false|-?\d+(?:\.\d+)?|\w+)`, 'g');
    let p;
...
function buildModel(content, boundVars, types, vars, notes, shape = false) {
  ...
  for (const m of content.matchAll(new RegExp(String.raw`^\s*(\w+)\s*=\s*(${LITERAL}|abap_true|abap_false|-?\d+(?:\.\d+)?)\s*\.\s*$`, 'gm'))) {
