...
export function elementBoundSlots(source) {
  ...
  for (const stmt of splitStatements(src)) {
    ...
    // the constant is reached through the handle (`client->cs_view-popup`),
    // through `me->client->…`, or through the interface itself
    // (`z2ui5_if_client=>cs_view-popup`) - all three are the same value
    const c = stmt.text.match(/\bview\s*=\s*(?:(?:\w+\s*->\s*)+|\w+\s*=>\s*)?cs_view-(\w+)/i);
    if (c) {
...
/** Whether an event name is a PER-KEYSTROKE wire: every `live*` event
 *  (liveChange, liveSearch, …), judged by prefix rather than by a list so a
 *  control's own spelling is covered. It is the test `live-event-roundtrip`
 *  applies to the `n =` of an attribute call, shared here so the rule that
 *  reads the wire's other half (`queue-last-without-no-busy`,
 *  lib/guide-rules.mjs) asks the same question of the same names. */
export const isLiveEvent = (name) => /^live\w*$/i.test(String(name ?? ''));
