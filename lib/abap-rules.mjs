...
import { manualInitFlag, viewNeverDisplayed, checkLifecycleDispatch } from './lifecycle-rules.mjs';
...
function checkEnumRowLiterals({ src, enumFields, report }) {
  if (!enumFields || !enumFields.size) return;
  ...
  for (const m of s.matchAll(/\bVALUE\s+(?:#|[A-Za-z_][\w/]*)\s*\(/g)) {
    ...
    if (rows.length) {
      ...
      for (const r of rows) {
        ...
        if (!topAssigned(r.body).size) continue;
        judge(key, r.body, open + 1 + r.offset, carried);
      }
      continue;
    }
...
function checkEventArgCallbacks({ src, report }) {
  for (const m of src.matchAll(/\bt_arg\s*=\s*VALUE\s+#?\s*\(/g)) {
    ...
    for (const el of body.matchAll(/\(\s*(?:([`'])([^`']*)\1|\|([^|]*)\|)\s*\)/g)) {
      const raw = (el[3] !== undefined ? el[3] : el[2]) ?? '';
      /* Only an argument the frontend RESOLVES is parsed as an expression. One
       * that does not open with $ or { is shipped as a JS string literal, so
       * the word "function" inside a toast template is not this rule's
       * business. */
      const lit = raw.trim();
      if (!lit.startsWith('$') && !lit.startsWith('{') && !lit.startsWith('\\{')) continue;
      const hit = lit.match(/\bfunction\s*\(|=>/);
      if (!hit) continue;
      report({
        type: 'event-arg-js-callback', value: lit.slice(0, 60),
        offset: open + 1 + el.index,
      });
    }
  }
}
...
function checkEventArgs({ src, report }) {
  ...
  for (const ev of src.matchAll(/client->_event\s*\(/g)) {
    ...
    /* `arg = <one value>` is the single-argument spelling, and the framework
     * APPENDS it to t_arg (z2ui5_cl_ui5_client~_event: `IF arg IS SUPPLIED.
     * APPEND … TO lt_arg.`), so it adds one to the arity rather than
     * replacing the table. The lookbehind is what keeps `t_arg` out: its
     * `arg` is preceded by `_`. Counting it is not cosmetic - without it
     * every wire migrated to the shorthand reads as "sends 0 t_arg" and
     * every get_event_arg( 1 ) behind it is reported out of range. */
    const am = /(?<![\w])arg\s*=(?!=)/.test(evBody);
    if (nameMatch) {
      let count = 0;
      if (tm) {
...
export function checkAbapRules(source, { data = null, controlIds = null, enumFields = null, minUi5 = '1.71', rules = null } = {}) {
  const src = scrub(source);
  ...
  checkEnumRowLiterals({ src, enumFields, report });
