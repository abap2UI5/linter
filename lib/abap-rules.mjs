 ...
 *
 * `enumFields` is what the VIEW exposes that way. Only an INSERT/APPEND of a
 * `VALUE #( … )` literal is judged: that is the construction site the port
 * writes by hand, and the one that omits a key. A row assembled field by field
 * or copied from another table has no single place to read. */
function checkEnumRowLiterals({ src, enumFields, report }) {
  ...
  for (const m of src.matchAll(/\b(?:INSERT|APPEND)\s+VALUE\s+#?\s*\(/gi)) {
    const open = src.indexOf('(', m.index + m[0].length - 1);
    const { body, end } = parenRegion(src, open);
    ...
    const tail = src.slice(end ?? open, (end ?? open) + 120);
    const target = tail.match(/\b(?:INTO\s+TABLE|TO)\s+(?:<\w+>-)?(\w+)/i);
    if (!target) continue;
    const fields = enumFields.get(target[1].toUpperCase());
    if (!fields) continue;
    const set = new Set();
    for (const f of body.matchAll(/(\w+)\s*=/g)) set.add(f[1].toUpperCase());
    for (const field of fields) {
      if (set.has(field) || omitted.has(field)) continue;
      report({ type: 'enum-field-unset-on-insert', member: field, offset: m.index });
    }
  }
}
