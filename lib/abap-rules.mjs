...
function eventScopes(src) {
  ...
  /* A WHEN literal names an event only inside a CASE over the EVENT. Read
   * from any CASE it made `WHEN \`A\`` in a `CASE mv_status` the handler
   * scope of event A, and a get_event_arg( 2 ) three lines later was judged
   * against A's arity - a false error on a status switch inside B's handler.
   * The CASE's own ENDCASE closes the scope again. */
  const cases = [...src.matchAll(CASE_OVER_EVENT)].map((m) => ({ from: m.index, to: m.index + m[0].length }));
  for (const m of src.matchAll(/\bWHEN\s+[`'|]([A-Z0-9_]+)[`'|]/gi)) {
    if (cases.some((c) => m.index > c.from && m.index < c.to)) scopes.push({ at: m.index, event: m[1].toUpperCase() });
  }
...
function checkAbapBooleans({ src, d, report }) {
  ...
  for (const m of src.matchAll(new RegExp(
    `->\\s*(${d.att})\\s*\\(\\s*n\\s*=\\s*[\`'|]([\\w:]+)[\`'|]\\s*(v)\\s*=\\s*([^)]*?)\\s*\\)`, 'gdi'))) {
    ...
    const isAbapBool =
      ...
      /^client->check_\w+\s*\($/i.test(value) ||
...
export function checkAbapRules(source, { data = null, controlIds = null, enumFields = null, boolFields = null, minUi5 = '1.71', rules = null } = {}) {
  ...
  for (const m of src.matchAll(new RegExp(
    `->\\s*(?:${d.att})\\s*\\(\\s*n\\s*=\\s*[\`'|]liveChange[\`'|]\\s+v\\s*=\\s*(?:me->)?client->_event\\s*\\(`, 'gi'))) {
