...
import { ACTION_ARGS, ID_ADDRESSED_ACTIONS, deniedControlMethod } from './frontend-actions.mjs';
...
export function checkAbapRules(source, { data = null, controlIds = null } = {}) {
  ...
  for (const call of src.matchAll(/client->(?:_event_client|follow_up_action)\s*\(/g)) {
    ...
    /* The same unknown-id silence for the other id-addressed actions —
     * except quieter: SET_FOCUS, SCROLL_TO and SCROLL_INTO_VIEW resolve
     * their id and `return` on a miss without even a console line, and
     * KEYBOARD_SET_MODE does the same. Judged under the identical viewIds
     * trust condition as CONTROL_BY_ID above. */
    if (ID_ADDRESSED_ACTIONS.has(action) && viewIds && args[0]?.value && !viewIds.has(args[0].value)) {
      report({
