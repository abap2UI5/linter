...
function checkAbapHygiene({ src, source, report }) {
  ...
  /* A client handle — the string `_bind( )`, `_bind_edit( )`, `_event( )` or
   * `_event_client( )` returns — captured into a variable instead of written
  ...
  for (const st of statements) {
    const m = st.text.match(/^\s*(?:DATA\(\s*(\w+)\s*\)|(\w+))\s*=\s*(?:me->)?client->(_(?:bind(?:_edit)?|event(?:_client)?))\s*\(/i);
    if (!m) continue;
...
function checkAttributeText({ src, d, boolVars, report }) {
...
export function checkAbapRules(source, { data = null, controlIds = null, enumFields = null, boolFields = null, minUi5 = '1.71', rules = null } = {}) {
  ...
  for (const m of code.matchAll(/client->_bind(?:_edit)?\s*\(\s*(?:val\s*=\s*)?(\w+)\s*[)\s]/gi)) {
  ...
  for (const lp of code.matchAll(/\bLOOP\s+AT\s+((?:me->)?[\w]+(?:-\w+)*)\s+(?:(?:REFERENCE\s+)?INTO|ASSIGNING)\s+(?:DATA\(\s*(\w+)\s*\)|FIELD-SYMBOL\(\s*(<\w+>)\s*\)|(<\w+>|\w+))/gi)) {
    ...
    for (const m of block.matchAll(/client->_bind(?:_edit)?\s*\(/gi)) {
  ...
  if (/\bPUBLIC\s+SECTION\b/i.test(src)) {
    ...
    for (const m of code.matchAll(/client->_bind(?:_edit)?\s*\(\s*(?:val\s*=\s*)?((?:me->)?\w+(?:-\w+)*)/gi)) {
  ...
  checkAttributeText({ src, d, boolVars, report });
   ...
   * the class's own declarations, so a plain data attribute never matches. */
  {
    ...
    if (refVars.size) {
      for (const m of code.matchAll(/client->_bind(?:_edit)?\s*\(\s*(?:val\s*=\s*)?((?:me->)?(\w+))\s*(?![-\w>])/gi)) {
  ...
  for (const mm of src.matchAll(/\bMETHOD\b[\s\S]*?\bENDMETHOD\b/gi)) {
    ...
    const dlg = [...mm[0].matchAll(new RegExp(`->\\s*(?:${d.open}|${d.leaf})\\(\\s*(?:n\\s*=\\s*)?\`(\\w+)\``, 'gi'))]
      .find((c) => c[1] === 'Dialog');
    if (!dlg) continue;
    if (/\b_event\s*\(|\b_event_client\s*\(|\bfollow_up_action\s*\(/i.test(mm[0])) continue;
