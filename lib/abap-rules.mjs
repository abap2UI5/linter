...
export function namedModels(source) {
  ...
  for (const call of src.matchAll(/client->(?:_event_client|follow_up_action)\s*\(/g)) {
    ...
    if (!/cs_event-set_odata_model\b/i.test(body)) continue;
...
export function checkAbapRules(source, { data = null, controlIds = null } = {}) {
   ...
   * z2ui5_cl_core_client, kept in the interface only so existing apps keep
  ...
  /* _event_frontend is obsolete - follow_up_action is the same call under its
   * current name (val / view / t_arg, and it returns the handler expression
   * too, so the rename holds in a view attribute as well). Renaming also puts
   * the call back within reach of the frontend-wire rules below, which key on
   * the current names: until then its t_arg is judged by nothing. */
  for (const m of src.matchAll(/client->(_event_frontend)\s*\(/g)) {
    const token = m.index + m[0].indexOf(m[1]);
    report({
  ...
  for (const m of src.matchAll(/->\s*a\s*\(\s*n\s*=\s*[`'|]liveChange[`'|]\s+v\s*=\s*client->_event\s*\(/g)) {
  ...
  for (const call of src.matchAll(/client->(_event_client|follow_up_action)\s*\(/g)) {
    ...
    if (!token) {
       ...
       *
       * Anything NOT name-shaped splits by method. follow_up_action inserts
       * it VERBATIM as custom_js — the raw-JavaScript escape hatch, which
       * runs unchecked in the browser and is reported as such: the frontend
       * is a renderer, logic that needs code belongs in ABAP or in a
       * cs_event- action that travels as data. _event_client has no such
       * path — its val is always an action name — so a non-name literal
       * there can never dispatch and is an unknown action. */
      const lit = callBody.match(/^\s*(?:val\s*=\s*)?(?:`((?:[^`]|``)*)`|'((?:[^']|'')*)'|\|((?:[^|\\]|\\.)*)\|)/);
      if (!lit) continue;
      const text = lit[1] ?? lit[2] ?? lit[3];
      if (!/^[A-Za-z0-9_]+$/.test(text)) {
        if (call[1] === 'follow_up_action') {
          report({
