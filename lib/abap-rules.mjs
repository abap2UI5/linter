...
import { checkChainLayout } from './chain-layout.mjs';
...
export function checkAbapRules(source, { data = null, controlIds = null } = {}) {
   ...
   * (`->att( )` in the successor builder's dialect).
   ...
   * VISIBLE. The classic silent inversion. Each builder has its own way out:
   * z2ui5_cl_ai_xml wraps the flag in as_bool( ), the successor takes it
   * through `att( b = )`, which renders true/false itself. */
  ...
  /* ->a( n = `x` v = <expr> ) with the value not wrapped in as_bool( ) — and
   * the same in the successor's dialect, where the `v` token itself is what
   * the correction replaces. A flag passed through `b =` never matches: the
   * regex asks for `v =`. */
  for (const m of src.matchAll(new RegExp(
    ...
    if (isAbapBool) {
       ...
       *
       * The correction differs per builder, and so does its SPAN: on
       * z2ui5_cl_ai_xml the value is wrapped in as_bool( ), on the successor
       * the parameter NAME becomes `b` and the value stays untouched — the
       * shorter and, on a class that already lives on the new builder, the
       * only correct one (it has no as_bool). */
      const bare = /^\w+$/.test(value);
      const fixes = !bare ? null
        : builder.bool
          ? [{ start: m.indices[3][0], end: m.indices[3][1], text: builder.bool }]
          : [{ start: m.indices[4][0], end: m.indices[4][1], text: `${builder.class}=>as_bool( ${value} )` }];
      report({
