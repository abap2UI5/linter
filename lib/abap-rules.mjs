...
export function checkAbapRules(source, { data = null, controlIds = null } = {}) {
  ...
  for (const call of src.matchAll(/client->(?:_event_client|follow_up_action)\s*\(/g)) {
    ...
    if (!token) {
      /* No cs_event constant: the action may be spelled as a string LITERAL,
       * which the server treats exactly like the constant — any name-shaped
       * value (^[A-Za-z0-9_]+$) is dispatched as a framework event. The
       * dispatch table misses with NO log at all, and the constants are
       * compile-checked while a literal is not, so only literals need the
       * existence check. Anything not name-shaped is follow_up_action's raw-
       * JavaScript escape hatch and is not judged. Case-sensitive on purpose:
       * the runtime never upper-cases, so `set_title` misses at runtime too. */
      const lit = callBody.match(/^\s*(?:val\s*=\s*)?([`'])([^`']*)\1/);
      if (!lit || !/^[A-Za-z0-9_]+$/.test(lit[2])) continue;
      if (!FRONTEND_EVENTS.includes(lit[2]) && !FRONTEND_EVENT_ALIASES.includes(lit[2])) {
        report({
          type: 'unknown-frontend-action', value: lit[2],
          allowed: [...FRONTEND_EVENTS].sort(), offset: base + lit.index,
        });
        continue;
      }
      action = lit[2].toLowerCase();
    }
