...
function checkDateFormatters({ src, report }) {
  ...
  for (const m of src.matchAll(/\b(\w+)\s+TYPE\s+(dats|tims|d|t)\b(?![\w-])/gi)) {
...
function checkBacktickEscapes({ src, report }) {
  const ESCAPE = /(?<!\\)\\[nrt]/;
...
function checkAbapHygiene({ src, report }) {
  ...
  for (const m of src.matchAll(/\bVALUE\s+(?:#|[A-Za-z_][\w/]*)\s*\(/g)) {
    const open = src.indexOf('(', m.index + m[0].length - 1);
    const { body } = parenRegion(src, open);
    if (body === undefined) continue;
    const rows = topRows(body);
  ...
  for (const m of src.matchAll(/\bINTO\s+CORRESPONDING\s+FIELDS\s+OF\s+TABLE\s+@DATA\(\s*(\w+)\s*\)/gi)) {
  ...
  const pub = src.match(/\bPUBLIC\s+SECTION\s*\.([\s\S]*?)(?=\b(?:PROTECTED\s+SECTION|PRIVATE\s+SECTION|ENDCLASS)\b)/i);
  for (const m of src.matchAll(/\bCLASS-METHODS\s+class_constructor\b/gi)) {
    const inPublic = pub && m.index > pub.index && m.index < pub.index + pub[0].length;
    if (!inPublic) {
      report({ type: 'class-constructor-visibility', member: 'class_constructor', offset: m.index });
    }
  }
  ...
  /* `DELETE itab INDEX sy-tabix` lexically inside a `LOOP AT` over the SAME
   * table. Two failure modes and only the loud one is visible: deleting the
   * current row shifts everything behind it while the loop cursor walks on
   * (the row after each deletion is silently skipped), and a sy-tabix reset
   * by an inner construct makes the DELETE hit another table's index — or
   * index 0, which is the TABLE_INVALID_INDEX short dump a live app 500ed
   * with. Eight real occurrences across the ecosystem when it was swept.
   * ANY enclosing loop over the table counts, not only the innermost — the
   * inner-loop case is the one that deleted by another table's index.
   * `DELETE itab INDEX sy-tabix` right after a READ TABLE, outside a loop
   * over that table, is correct and common, and stays silent here. */
  {
    const events = [];
    ...
    for (const e of events) {
      ...
      else if (e.kind === 'close') stack.pop();
      else if (e.kind === 'reset') stack.length = 0;
      else if (stack.includes(e.name)) {
...
export function checkAbapRules(source, { data = null, controlIds = null, enumFields = null, boolFields = null, minUi5 = '1.71', rules = null } = {}) {
  ...
  for (const m of src.matchAll(/client->(_bind(?:_edit)?)\s*\(/g)) {
    const open = src.indexOf('(', m.index + m[0].length - 1);
    const { body } = parenRegion(src, open);
    if (body === undefined) continue;
    const base = open + 1;
  ...
  source.split('\n').forEach((line, i) => {
    ...
    const ws = /[ \t]+$/.exec(content);
    if (ws) {
      report({
        type: 'trailing-whitespace', member: String(i + 1), value: ws[0].length,
        offset: lineStart + ws.index,
        fixes: [{ start: lineStart + ws.index, end: lineStart + ws.index + ws[0].length, text: '' }],
      });
    }
  ...
  manualInitFlag({ src, report, boolVars, attrs, locals });
  lifecycleIsInitial({ src, source, report, boolVars });
  ...
  viewNeverDisplayed({ src, d, report });

  checkEventArgs({ src, source, report });
  checkEventArgCallbacks({ src, report });
  checkDateFormatters({ src, report });
  checkBacktickEscapes({ src, report });
  checkAbapHygiene({ src, report });
  ...
  const frontendRaised = checkFrontendWires({ src, report, viewIds, data, controlIds, minUi5 });
  ...
  checkLifecycleDispatch({ src, report });
