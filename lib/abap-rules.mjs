...
function privateInstanceAttributes(src) {
  ...
  const names = [];
  const body = collapseInlineStructures(section[1]);
  for (const m of body.matchAll(/^\s*DATA:?\s+([\s\S]*?)(?=^\s*(?:CLASS-)?DATA\b|^\s*(?:METHODS|CLASS-METHODS|TYPES|CONSTANTS|INTERFACES|ALIASES)\b|$)/gim)) {
    for (const declaration of m[1].split(',')) {
      const name = declaration.trim().match(/^(\w+)/);
      if (name) names.push({ name: name[1], offset: bodyAt + body.indexOf(name[1], m.index) });
    }
  }
  return names;
}
...
function checkAbapHygiene({ src, report }) {
  ...
  if (intTargets.size) {
    for (const m of src.matchAll(/^[ \t]*([a-z_]\w*)[ \t]*=[ \t]*CONV[ \t]+i\([^()|]*\)[ \t]*\.[ \t]*$/gim)) {
      if (!intTargets.has(m[1].toLowerCase())) continue;
      report({ type: 'redundant-conv-i', member: m[1], offset: m.index });
    }
  }
...
export function checkAbapRules(source, { data = null, controlIds = null, enumFields = null, boolFields = null, minUi5 = '1.71', rules = null } = {}) {
  ...
  lifecycleIsInitial({ src, report, boolVars });
