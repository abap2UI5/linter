 ...
 *   allClasses   bool   - true collects every .clas.abap; a class that builds no
 *                         view is judged by the source-side rules alone
 *                         (= --all-classes)
...
import { RULES_PAGE } from './rule-docs.mjs';
...
export const KNOWN = new Set(['$schema', 'extends', 'paths', 'ignore', 'ui5', 'minUi5', 'distribution', 'allow', 'render', 'properties', 'allClasses', 'cache', 'failOn', 'maxWarnings', 'rules', 'baseline', 'badge']);
...
function loadRules(file, raw) {
  ...
  for (const [id, value] of Object.entries(raw)) {
    ...
    if (!RULES.includes(id) && id !== RENDER_RULE) {
      ...
      const meant = caseMatch(id, [...RULES, RENDER_RULE]);
      throw new Error(`${file}: unknown rule '${id}' in 'rules'${meant ? ` - did you mean '${meant}'?` : ''}`
...
function validate(file, raw) {
  ...
  if (raw.allClasses !== undefined) {
    if (typeof raw.allClasses !== 'boolean') throw new Error(`${file}: 'allClasses' must be true or false`);
    cfg.allClasses = raw.allClasses;
  }
