 ...
 *   badge        string or { file, label, logo, labelColor } - where to write
 *                         the shields.io endpoint JSON for the run
 *
...
const BADGE_KEYS = ['file', 'label', 'logo', 'labelColor'];

/** `badge` is a path, or that path plus how the badge should look. Both
 *  spellings normalize to the object form the CLI writes from. */
function loadBadge(file, raw) {
  ...
  if (typeof badge !== 'object' || badge === null || Array.isArray(badge)) {
    throw new Error(`${file}: 'badge' must be a file path or { ${BADGE_KEYS.join(', ')} }`);
  }
