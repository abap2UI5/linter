 ...
 * in ONE upstream commit, and the same commit moved `z2ui5_cl_ai_xml` and
 * `z2ui5_cl_http_handler` into the frozen package. An app that names an
 * object from either side compiles today and fails to activate after the
 * next `abapGit` pull, with no deprecation in between.
 *
 ...
 *
 *   RELEASED_OBJECTS   src/02 — the whole allowed surface, 5 objects
 ...
 *
 * `tolerated` marks the two the rule stays silent about, each for a reason
 * that has nothing to do with them being good code:
 *
 *   z2ui5_cl_ai_xml   is the builder this linter READS. `collectFiles` keys
 *                     on `z2ui5_cl_ai_xml=>factory` and `reconstruct.mjs`
 *                     re-executes its chains, so every file the linter looks
 *                     at names it by construction — reporting it would put
 *                     one warning on 100% of the corpus and tell the author
 *                     to migrate to a builder this linter cannot read at all
 *                     (`z2ui5_cl_ui5_view_builder`; supporting it is the
 *                     reconstructor project in AGENTS.md). The moment that
 *                     lands, drop the flag: it is the whole reason it exists.
 *   z2ui5_if_types    is still part of the released signature. The RELEASED
 *                     `z2ui5_if_client~get( )` returns
 *                     `z2ui5_if_types=>ty_s_get`, so an app that stores that
 *                     result in a declared variable cannot avoid the name.
 *                     Frozen or not, an app is not the one to fix that.
 */
export const FROZEN_OBJECTS = Object.freeze({
  // package top level — the two superseded view builders and the old entry point
  z2ui5_cl_ai_xml: { area: 'src/99', replacement: 'z2ui5_cl_ui5_view_builder', tolerated: true },
  ...
  z2ui5_if_types: { area: 'src/99', replacement: null, tolerated: true },
 ...
 *
 *   null                     released, tolerated, or not a framework object
...
export function apiVerdict(objectName) {
  ...
  if (frozen) {
    if (frozen.tolerated) return null;
