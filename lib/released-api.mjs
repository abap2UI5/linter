 ...
 *
 * `tolerated` marks the one object the rule stays silent about, for a reason
 * that has nothing to do with it being good code:
 *
 ...
 *
 * `z2ui5_cl_ai_xml` used to be tolerated for a second reason — it was the
 * only builder the reconstructor could read, so reporting it would have put
 * one warning on 100% of every corpus and pointed the author at a class this
 * linter was blind to. That reason is gone: `lib/builders.mjs` reads both
 * dialects, and the successor named in the finding is the one the gates
 * understand best. The exemption went with it.
 */
export const FROZEN_OBJECTS = Object.freeze({
  // package top level — the two superseded view builders and the old entry point
  z2ui5_cl_ai_xml: { area: 'src/99', replacement: 'z2ui5_cl_ui5_view_builder' },
