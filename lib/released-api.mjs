...
export const RELEASED_OBJECTS = Object.freeze([
  ...
  'z2ui5_if_client',
  'z2ui5_if_ui5_exit',
  // Released 2026-09 to close a docs/removal-plan.md item: the two-field
  // structure a dynamic CREATE DATA can name without touching the framework's
  // own draft table. Listed BEFORE the prefix families match them — the
  // whitelist wins, so it does not fall into the z2ui5_t_ table family below.
  //
  // A released JSON reader, z2ui5_cl_ui5_json, sat here beside it for two
  // weeks and is GONE: abap2UI5 removed it on 2026-09-14 before it had
  // shipped in any release tag, so no installation ever had it and listing it
  // here would let a call to a non-existent class pass this rule. JSON is
  // built and read by hand now — see the remedy of `non-released-api`.
  'z2ui5_t_02',
]);
