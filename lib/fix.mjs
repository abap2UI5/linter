...
export const FIXABLE = Object.freeze(['obsolete-binder', 'obsolete-bind-argument', 'obsolete-model-update', 'obsolete-frontend-event', 'unconverted-abap-boolean', 'unescaped-text-in-attribute', 'event-arg-unresolved', 'popover-display-val', 'undeclared-namespace', 'literal-view-slot', 'chain-house-layout', 'escaped-brace-in-backtick', 'lifecycle-is-initial', 'redundant-conv-i', 'redundant-serializable', 'trailing-empty-event-arg', 'byte-order-mark', 'crlf-line-ending', 'trailing-whitespace', 'missing-final-newline', 'redundant-init-display', 'binding-to-reference', 'unescaped-brace-in-style', 'collapsed-brace-in-style', 'class-constructor-visibility', 'escape-sequence-in-backtick', 'json-bind-on-scalar-property', 'unknown-control', 'unknown-property', 'unknown-aggregation', 'invalid-property-value', 'unknown-event-parameter', 'unknown-icon', 'frontend-action-unknown-id', 'popover-anchor-unknown-id', 'event-name-case-mismatch', 'unused-namespace-declaration', 'event-arg-default-index', 'event-arg-single-row-table', 'missing-on-navigated-branch', 'separate-lifecycle-ifs', 'binding-to-nonpublic', 'private-app-attribute', 'hardcoded-binding-path', 'external-link-without-target', 'unknown-model', 'uncurated-formatter', 'unknown-frontend-action', 'client-handle-capture', 'missing-view-display-on-navigated', 'unknown-binding-path', 'excess-shut', 'insecure-asset-url', 'loop-work-area-bound', 'live-event-roundtrip', 'frontend-action-as-backend-event', 'popup-display-xml', 'queue-last-without-no-busy', 'nest-view-without-destroy', 'unknown-directive-rule', 'unused-directive']);
...
export function applyFixes(source, findings) {
  // each edit remembers the finding it came from, so the caller can say WHICH
  // findings a pass (or a dry run) settled - additive key `findings` below
  const all = findings.flatMap((f) => (f.fixes ?? []).map((e) => ({ ...e, finding: f })));
  ...
  for (const e of edits) {
    ...
    cursor = e.end;
    applied++;
    settled.add(e.finding);
  }
  return { output: out + source.slice(cursor), applied, deferred, dropped, findings: [...settled] };
}
