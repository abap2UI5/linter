...
import { SEVERITIES, severityOf, describe, RENDER_RULE, defaultSeverityOf } from './findings.mjs';
import { RULE_DOCS } from './rule-docs.mjs';
...
export function badgeEndpoint(summary, stats, opt = {}) {
  ...
  const state = summary.totals.error ? plural(summary.totals.error, 'error')
    : problems ? plural(problems, 'problem')
      : 'clean';
  /* The REACH of the check, in the units the summary counts: how many classes
   * were read, how many views came out of them, how many controls those were
   * made of. "clean" alone says a gate ran; these say how much it ran over,
   * which is the number that grows with the corpus and that no workflow badge
   * can carry. A segment with nothing to say (an XML-only corpus has no app
   * classes) is left out rather than printed as a zero. */
  ...
  if (!reach.length) reach.push(count(summary.files, 'file'));
  ...
  /* The split follows what the two halves MEAN: everything factual - the
   * linter's name and the reach - sits grey on the left, and the right half
   * carries the verdict alone, in the colour of that verdict. Read at badge
   * size that is one green (or red) word against a neutral sentence, instead
   * of a colour bar whose text has to be read to the end. */
  const name = opt.label ?? 'abap2UI5-linter';
  return {
    schemaVersion: 1,
    label: nothing ? name : `${name} ${reach.join(' · ')}`,
    message: nothing ? 'nothing checkable' : state,
    color: nothing ? '9f9f9f' : summary.totals.error ? 'e05d44' : problems ? 'dfb317' : '4c1',
    labelColor: opt.labelColor ?? '555',
    // no logo by default - a logo next to a sentence-long label only crowds it
