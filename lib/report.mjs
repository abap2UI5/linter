...
export function statsRows(stats, summary, opt = {}) {
  ...
  if (stats.abap) {
    ...
    const helpers = stats.abap - stats.builder;
    sources.push(helpers
      ? `${num(stats.abap)} classes (${num(stats.builder)} app class${stats.builder === 1 ? '' : 'es'} building a view, ${num(helpers)} helper${helpers === 1 ? '' : 's'})`
      : `${num(stats.abap)} app class${stats.abap === 1 ? '' : 'es'}`);
  }
