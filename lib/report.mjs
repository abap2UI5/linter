...
import { SEVERITIES, severityOf, describe, RENDER_RULE, RULES, defaultSeverityOf, OPT_IN, isOptInEnabled } from './findings.mjs';
import { RULE_DOCS, CATEGORIES, RULES_PAGE, ruleUrl, ruleExampleUrl } from './rule-docs.mjs';
import { FIXABLE } from './fix.mjs';
...
export function runStats(results) {
  const s = {
    ...
    /* app classes whose view is built in ANOTHER class - judged by the
     * source-side and lifecycle rules only, so a clean verdict on one is not
     * a verdict on a view */
    appsWithoutView: 0,
  ...
  for (const r of results) {
    if (r.kind === 'xml') s.xml++;
      ...
      if (r.appWithoutView) s.appsWithoutView++;
    }
    const st = r.stats;
    if (st) {
...
export function statsRows(stats, summary, opt = {}) {
  ...
  const sources = [];
  if (stats.abap) {
    /* Under --all-classes most of what was read is helpers: `106 app classes
     * (2 building a view)` called 104 helper classes app classes. The count
     * says what each kind is now, and the plain corpus keeps its one phrase;
     * an app class whose view comes from another class is named as such. */
    const helpers = stats.abap - stats.builder - (stats.appsWithoutView || 0);
    const shares = [];
    if (stats.appsWithoutView) shares.push(`${num(stats.appsWithoutView)} building none here - the view comes from another class`);
    if (helpers > 0) {
      sources.push(`${num(stats.abap)} classes (${num(stats.builder)} app class${stats.builder === 1 ? '' : 'es'} building a view, `
        + shares.map((s) => `${s}, `).join('') + `${num(helpers)} helper${helpers === 1 ? '' : 's'})`);
    } else {
      if (stats.builder !== stats.abap) shares.unshift(`${num(stats.builder)} building a view`);
...
function checksMessage(summary, opt) {
  ...
  // an OPT-IN rule runs only when the rules block asks for it, so unasked it
  // is not a rule that passed - it is a rule that did not run
  const ran = RULES.filter((id) => opt.rules?.[id] !== false && (!OPT_IN.has(id) || isOptInEnabled(opt.rules, id))).length;
...
export function formatStylish(results, summary, opt = {}) {
  ...
  for (const r of results) {
    ...
    for (const p of problems) {
      ...
      /* …and the rule's card after the id, on every line: an id is what a
       * reader greps, a link is what they click, and the reference block at
       * the end is too far from the line for a log of any length. */
      out.push(
        `  ${paint('dim', where.padEnd(width))}  ${paint(SEVERITY_COLOR[p.severity], p.severity.padEnd(7))}  ` +
        `${p.message.padEnd(msgWidth)}  ${paint('dim', p.rule)}  ${paint('dim', ruleUrl(p.rule))}`.trimEnd()
      );
    }
    out.push('');
  }
  ...
  /* Which config the verdict was reached under, and what its `ignore` kept
   * out of the walk - two facts a reader otherwise reconstructs by hand when
   * a run reports less than expected. Under the count line like the run
   * summary, and like it never inside a machine format (the CLI hands
   * `config` in for the stylish report only). */
  if (opt.config?.file) {
    out.push(paint('dim', `config: ${opt.config.file}`));
    if (opt.config.ignored) out.push(paint('dim', `${plural(opt.config.ignored, 'file')} ignored by config`));
  }
...
/*
 * The rule reference, in the terminal.
 *
 * Every reported line ends in a rule id and the card's URL, and the block
 * under the count line lists both again — but a reader at a terminal (the
 * Eclipse ADT developer inside `--watch`, someone reading a CI log) still has
 * to open a browser for the paragraph. RULE_DOCS is the same prose the page
 * is generated from and the same prose mcp-server hands an agent, so the
 * third reader gets it the same way: `abap2ui5lint --explain <id>`.
 */

/** Every id the rules page has a card for, in the page's order: the
 *  registry plus the render gate's pseudo-rule, category by category as
 *  CATEGORIES declares them, alphabetical within one. */
export function ruleIndex() {
  const all = [...RULES, RENDER_RULE].sort();
  return CATEGORIES.map((cat) => ({ ...cat, ids: all.filter((id) => RULE_DOCS[id]?.category === cat.id) }));
}

/* A paragraph at terminal width. RULE_DOCS holds each one as a single
 * string, which is what the page and a JSON consumer want; a terminal wants
 * lines. Broken between words only, so a `code span` stays whole unless it is
 * itself wider than the column. */
const wrap = (text, width, indent = '  ') => {
  const lines = [];
  let line = '';
  for (const word of String(text).split(/\s+/).filter(Boolean)) {
    if (line && `${indent}${line} ${word}`.length > width) { lines.push(indent + line); line = word; }
...
/**
 * `--explain <id>…`: the id with its default severity (and whether `--fix`
 * knows it), the summary, the detail paragraph, the fix note where the rule
 * has one, the before/after pair, and the card's URL. Every id must be one
 * `ruleIndex( )` lists — the CLI validates first, so nothing is half printed.
 */
export function formatExplain(ids, opt = {}) {
  const paint = painter(opt.color ?? colorEnabled());
  const width = opt.width ?? 78;
  const code = (text) => text.split('\n').map((l) => `    ${l}`);
  const out = [];
  for (const id of ids) {
    const doc = RULE_DOCS[id];
    const badges = [defaultSeverityOf(id), ...(FIXABLE.includes(id) ? ['--fix'] : [])];
    out.push(`${paint('bold', id)}  ${paint('dim', `(${badges.join(', ')})`)}`);
    out.push(...wrap(doc.summary, width));
    out.push('', ...wrap(doc.detail, width));
    if (doc.fixNote) out.push('', ...wrap(`--fix: ${doc.fixNote}`, width));
    if (doc.example) {
      out.push('', `  ${paint('dim', 'reported:')}`, ...code(doc.example));
      if (doc.remedy) out.push('', `  ${paint('dim', 'fixed:')}`, ...code(doc.remedy));
    }
    out.push('', `  ${paint('dim', ruleUrl(id))}`, '');
  }
  return out.join('\n').trimEnd();
}

/**
 * `--explain` with no id: every rule id with its summary, one per line, under
 * the page's category headings and in the page's order — so the terminal and
 * the browser agree on where a rule is found.
 */
export function formatRuleIndex(opt = {}) {
  const paint = painter(opt.color ?? colorEnabled());
  const index = ruleIndex();
  const w = Math.max(...index.flatMap((c) => c.ids).map((id) => id.length));
  const out = [];
  let n = 0;
  for (const cat of index) {
    if (!cat.ids.length) continue;
    out.push(paint('bold', cat.title));
    for (const id of cat.ids) {
      out.push(`  ${id.padEnd(w)}  ${paint('dim', RULE_DOCS[id].summary)}`);
      n++;
    }
    out.push('');
  }
  out.push(paint('dim', `${plural(n, 'rule')} - abap2ui5lint --explain <id> for one, ${RULES_PAGE} for the page`));
  return out.join('\n');
}

/**
 * One line on stderr after a report with findings, naming the command that
 * explains them: `abap2ui5lint --explain <id> <id> <id> ...` — the first
 * three ids the run reported, in the order a reader meets them. Only where a
 * person is reading (a terminal), never in --quiet and never beside a
 * machine format; null otherwise. Stderr, like the progress line, so stdout
 * stays the report.
 */
export function explainFooter(results, { quiet = false, format = 'stylish', tty = false, limit = 3 } = {}) {
  if (!tty || quiet || format !== 'stylish') return null;
  const ids = rulesReported(results, quiet).map((r) => r.rule);
  if (!ids.length) return null;
  return `abap2ui5lint: what each id means, in the terminal: abap2ui5lint --explain ${ids.slice(0, limit).join(' ')}${ids.length > limit ? ' ...' : ''}`;
}
