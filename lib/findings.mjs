...
const SEVERITY_BY_TYPE = {
  ...
  /* An error rather than a warning, and the only rule whose severity is about
   * what was NOT checked: a class on the frozen builder reconstructs no view,
   * so every other rule here was silent about it for lack of anything to read.
   * A repo that maintains such an app deliberately says so once:
   * `"rules": { "frozen-view-builder": "warning" }`. */
...
export function describe(f) {
  switch (f.type) {
    ...
    case 'frozen-view-builder':
      return `this class builds its view with ${f.value}, the frozen predecessor of z2ui5_cl_ui5_view_builder — so NOTHING about the view was checked here: no control, no property, no binding, no render. It is also outside the released API (src/99, kept only so existing installations keep compiling), so the next abapGit pull may take it away. Rewrite the chain on z2ui5_cl_ui5_view_builder (verbs: ele, tag, a, end, stringify)`;
