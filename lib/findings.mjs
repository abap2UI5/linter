...
export function attachSourceFixes(findings, source) {
  return attachJsonBindFixes(attachNamespaceFixes(findings, source), source);
}
...
export function describe(f) {
  switch (f.type) {
    ...
    case 'unknown-property':
      return `${f.control} has no property/event/association ${f.member} — typo?${snapshotHint(f)}`;
    case 'unknown-aggregation':
      return `${f.control} has no aggregation ${f.member} — typo?${snapshotHint(f)}`;
