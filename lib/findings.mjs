...
export function applyRules(findings, rules, file = '') {
  /* The path an `exclude` is matched against is the one the runner reached
   * the file by, and with `paths` in a config file that is ABSOLUTE - while
   * the report prints it relative to the cwd. So `"exclude": ["^src/00/98/"]`,
   * written from what you can read in the report, silently matched nothing.
   * A silent exclusion is the worst kind: the config says the rule is waived
   * there, the findings keep coming, and nothing connects the two. Both forms
   * are tried, so a pattern written against either works. */
  const asShown = file && path.isAbsolute(file) ? path.relative(process.cwd(), file) : file;
  const excluded = (cfg) => cfg.exclude?.some((re) => re.test(file) || re.test(asShown));
  const kept = [];
  for (const f of findings) {
