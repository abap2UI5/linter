...
export function attachNamespaceFixes(findings, source) {
  for (const f of findings) {
    ...
    if (!lib) continue;
    // written in the dialect the class is on: `)->a( ` or `)->att( `
    const anchor = String(source).match(/->\s*(att|a)\(\s*(?=n\s*=\s*`xmlns`)/);
    if (!anchor) continue;
  ...
  switch (f.type) {
    ...
    case 'unconverted-abap-boolean':
      return `${f.member}: the ABAP boolean ${f.value} reaches the view as 'X'/' ' — ${
        f.fixHint || 'wrap it in z2ui5_cl_ai_xml=>as_bool( )'}`;
    ...
    case 'chain-indentation': {
      ...
      const why = ' — the chain\'s indentation is the only picture of the view\'s tree there is, and this line draws a different one than the view that renders';
    ...
    case 'chain-call-per-line':
      return `${f.count} builder calls on one line of a multi-line chain — one call per line, or the indentation stops showing the tree`;
    case 'non-released-api':
      return `${f.value} is not part of abap2UI5's released API (src/02) — it lives in ${f.what} (${f.member}), ${
        f.frozen
          ? 'frozen legacy that ships only so existing installations keep compiling and is never fixed or extended'
          : 'which the framework renames and restructures without notice'}${
