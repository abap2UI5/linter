 ...
 */
/* The compiled-theme cache, per user and private.
 *
 * `/tmp/abap2ui5-theme-css` was one path shared by every account on the
 * machine. The first one to create it owns it, every other one then READS
 * whatever is in it, and anything that can write there decides what a
 * rendered preview looks like - on a shared build machine that is somebody
 * else's CSS in your screenshot. The uid in the name and 0700 below make the
 * cache the running user's own, and the sticky bit on /tmp keeps another
 * account from replacing a directory that already exists.
 *
 * The cost is one theme recompile the first time a version of the linter
 * runs under a new name; the cache is a speed cache and rebuilds itself. */
const cacheOwner = () => {
  try {
...
const THEME_CACHE = path.join(os.tmpdir(), `abap2ui5-theme-css-${cacheOwner()}`);
...
async function launchBrowser(launchOpts = {}) {
  ...
  try {
    ...
    for (const exe of [process.env.CHROMIUM_BIN, '/opt/pw-browsers/chromium', '/usr/bin/chromium', '/usr/bin/chromium-browser']) {
      if (exe && fs.existsSync(exe)) return chromium.launch({ ...launchOpts, executablePath: exe });
    }
    throw e;
  }
}
 ...
 *
 * `handleSIGINT` is Playwright's own option under the same name: by default
 * Ctrl+C is the browser's to answer - Playwright closes it and then EXITS
 * THE PROCESS with 130, whatever else the process meant to do. A caller that
 * owns the process's signal handling and keeps the renderer open across
 * calls (the CLI's --watch loop, which promises exit 0 on Ctrl+C) passes
 * false and closes the renderer itself.
 */
export async function openRenderer({ pages = 1, theme = GATE_THEME, css = false, handleSIGINT = true } = {}) {
  ...
  const browser = await launchBrowser({ handleSIGINT });
