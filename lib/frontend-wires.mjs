...
import {
  ...
  CS_VIEW_CONSTANT, FILTER_OPERATORS, GLOBAL_TARGET_SINCE, CONTROL_METHOD_KINDS, OBSOLETE_EVENT_CONSTANTS,
  URLHELPER_PARAMS,
...
export function checkFrontendWires({
  ...
  for (const call of (code ?? src).matchAll(/client->(_event_client|follow_up_action)\s*\(/gi)) {
    ...
    /* A constant upstream keeps under its `"obsolete` label. The wire still
     * works — its VALUE is remapped or accepted like any other — so nothing
     * below changes for it; this is the one finding that reads the NAME. */
    if (token && OBSOLETE_EVENT_CONSTANTS.includes(action)) {
      report({ type: 'obsolete-event-constant', member: action, offset: base + token.index });
    }
    if (!token) {
      ...
      if (!FRONTEND_EVENTS.includes(text) && !FRONTEND_EVENT_ALIASES.includes(text)
        ...
        /* The dispatch table is case-sensitive, so `set_title` is not
         * `SET_TITLE` — and that is the one miss a did-you-mean can name
         * without guessing (lib/suggest.mjs): the fix rewrites the literal. */
        const suggestion = caseMatch(text, [...FRONTEND_EVENTS, ...FRONTEND_EVENT_ALIASES, ...SERVER_EVENTS]);
        // the literal's content ends one character before the match does
        const at = base + lit.index + lit[0].length - 1 - text.length;
        report({
          type: 'unknown-frontend-action', value: text,
          allowed: [...FRONTEND_EVENTS].sort(), offset: base + lit.index,
          ...(suggestion ? { written: text, suggestion, fixes: [{ start: at, end: at + text.length, text: suggestion }] } : {}),
        });
        continue;
      }
    ...
    if (vm && !VIEW_SLOTS.includes(vm[3])) {
      // `main` for MAIN is a case miss, not a wrong slot: the fix rewrites it
      const suggestion = caseMatch(vm[3], VIEW_SLOTS);
      const at = base + vm.index + vm[0].indexOf(vm[1]) + 1;
      report({
        type: 'unknown-view-slot', member: 'view', value: vm[3], offset: base + vm.index,
        ...(suggestion ? { written: vm[3], suggestion, fixes: [{ start: at, end: at + vm[3].length, text: suggestion }] } : {}),
      });
    } else if (vm) {
        ...
        outer: for (const group of usable) {
          for (const row of group) {
            ...
            if (!FILTER_OPERATORS.includes(row[1])) {
              /* The operator set is case-sensitive upstream (`contains`
               * logs and leaves the binding unfiltered where `Contains`
               * filters), so a case miss is the one did-you-mean that is
               * not a guess; the fix rewrites the operator in the payload. */
              const suggestion = caseMatch(String(row[1]), FILTER_OPERATORS);
              report({
                ...
                ...(suggestion ? { written: String(row[1]), suggestion } : {}),
              });
              break outer;
            }
          }
